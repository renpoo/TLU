...
export TARGET_ENV="samples/Sample_8_fMRI_Stroke"
mkdir -p "$TARGET_ENV/input_stream"
mkdir -p "$TARGET_ENV/ephemeral"

${TLU_PY} -m src.filters._0_0_generate_dummy_fmri --pathology stroke --out-initial-state "$TARGET_ENV/ephemeral/_initial_state_labels.csv" \
