...
| **Sample 5** | 交通（都市交通網） | 京都の狭路信号大渋滞 | **循環不全・血栓 (Gridlock)** | `Spectral Radius` ＝ `1.0000` (デッドロック) |
...
| **Sample 9** | 脳科学（fMRI血流） | 側頭葉起点の発作波 | **神経過同期 (Seizure)** | `Spectral Radius` ＝ `1.0000` (脳全体共振) |
...
*   **Sample 1 (Wash Trade):** 売上高と資産が急拡大しているにもかかわらず、経費が完全に横ばいです。「運動量（売上）が増加しているのに燃料（経費）の消費が増えない」という物理的矛盾が、循環取引を瞬時に見抜くトリガーとなります。
...
*   **Sample 0 / 1 / 5 / 6 / 7 / 8 / 9:** `Leak Ratio` は完全に **`0.000`** です。車も血流も、そして循環取引も「お金やモノがシステム内で閉じている」ため、外部へ漏れ出てはいません。
*   **Sample 2 (Embezzlement):** `Leak Ratio` が **`0.000111`** となり、説明のつかない「質量の蒸発（内出血）」を検知。統計モデルが見逃した横領を、保存則が決定的に暴きます。
*   **Sample 3 (Bookkeeping Mistake):** `Leak Ratio` が **`0.000275` (保存残差 906.29)** に激増。片面記帳や入力漏れによる「急激な質量欠損」を記録します。
...
*   **Sample 1 (Wash Trade):** スペクトル半径が `0.7488` へ跳ね上がり、売上・売掛金・現金の位相差が完全にゼロで同調（Rigid Lock）しています。
*   **Sample 5 (Kyoto Traffic) & Sample 9 (fMRI Seizure):** スペクトル半径が限界値の **`1.0000`** に張り付きます。交通流は「デッドロック」、脳は「てんかん発作（過同期共振）」を起こし、ネットワーク全体がフリーズして正常な伝達機能を完全に失っています。
...
#### Sample 9 (fMRI Seizure) - てんかん発作により、スペクトル半径が `1.0000` へ固着
