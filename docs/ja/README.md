...
| **3** | **[03. Market Forensics & Compliance Rules](../../scratch/03_Market_Forensics_Rules_EN.md)** | **[03. 株式市場分析とコンプライアンス監査ルール](../../scratch/03_Market_Forensics_Rules.md)** | ミリ秒単位の注文簿（オーダーブック）力学、USR循環取引、二部グラフ投影 vs ユーザー間直接送金トポロジーによる共謀ネットワーク監査。 |
| **4** | **[LLM Diagnostic Manual (Supreme prompt & Operations)](../LLM_Diagnostic_Manual.md)** | **[LLM メタ検査マニュアル（最高メタレベルシステムプロンプト＆運用手順）](LLM_Diagnostic_Manual.md)** | 物理数学エンジンの数値からAIが客観的なカルテを自動生成するためのプロトコル。統計的偽陽性判定と、原本データへのファクトチェック義務化。 |
| **5** | **[Universal Forensic Cross-Verification Registry](../samples/README.md)** | **[数理解析ガイド＆検証サンプル総合目次](samples/README.md)** | TLUに実装されている全10種類の検証サンプルの判定判定、物理数学パラメータ限界値、および線形最適制御（LQR）介入ポイント・ガイド。 |
