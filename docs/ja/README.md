...
TLUには、物理数学エンジンの精度を実証するため、13のサンプルデータセットがパッケージ化されています。
...
| **10** | **[🟢 ERP製造間接費の伝統的配賦 (Traditional Allocation)](samples/Sample_10_ERP_Traditional/README.md)** | ERP会計 | **NORMAL (偏圧気滞)** | 労務時間拘束, エントロピー $S$=2.66 | 偏圧性気滞・血瘀 (製品Aへの偏圧) |
| **11** | **[🟢 ERP製造間接費の標準ABC配賦 (Standard ABC)](samples/Sample_11_ERP_ABC/README.md)** | ERP会計 | **NORMAL (経絡疎通)** | 固定3活動プール, エントロピー $S$=3.11 | 経絡疎通・動的脈動 (製品バランス是正) |
| **12** | **[🟢 ERP製造間接費の動的T-ABC配賦 (Dynamic T-ABC)](samples/Sample_12_ERP_TABC/README.md)** | ERP会計 | **NORMAL (陰陽平衡)** | 摩擦ロス動的自律分離 $\alpha(t)$, $S$=3.11 | 陰陽平衡・無駄熱(邪気)の自律排出 |
