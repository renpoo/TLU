...
* **総合体質診断（健康状態）:**
  本システムは「体格（総代謝血流量）」は `100000.00` で完全に保存されています（キルヒホッフ残差 `0.00` であり、外部への物理的出血はありません）。しかし、過同期発作の発生により、脳全体の自律的な活動分散（エントロピー `9.0982`）が極端に押し潰され、しなやかさを失った「動脈硬化（最大結合剛性 `0.0037` の同期ロック）」状態にあります。脳全体の体温（流量ボラティリティ）が特定の波によって一様に支配された、極めて危険な「機能的麻痺状態」です。高次微分ショック（Jerk/Snap）は過同期発作開始の t=30 を境に、サイン波の同期的な急変（自励ノッキング振動）として規則的に高騰しています。
* **要改善点と介入アドバイス:**
...
* **過同期ノッキング共振（Jerk `jerk_j` & Snap `snap_s`）:**
  - **数理的解釈:** 高次微分成分としての Jerk および Snap が発作開始の t=30 以降、規則的な周期インパルス（急変化ショック）を繰り返す状態で固定。滑らかな脳機能対流が消失し、てんかん発作特有の規則的な発振ノッキング状態にあることを証明しています。
...
本システムにおける対流動的データ（状態 `state_X`, 速度 `velocity_v`, 加速度 `acceleration_a`, 局所粘性 `viscosity_C`、および加加速度 `jerk_j`、加加加速度 `snap_s`）の記述統計量を以下に示します。データソースは [result.000_1_1_filter_dynamics.analysis.csv](../../../../samples/Sample_9_fMRI_Seizure/output_data/result.000_1_1_filter_dynamics.analysis.csv) です。
...
| **加加速度 jerk_j** | -0.0000 | 0.0000 | 0.0000 (38/300, 12.7%) | -245.4300 | 252.9100 | 498.3400 | 101.13 | 85.32 | 0.0506 | 0.5289 |
| **加加加速度 snap_s** | -0.0000 | 0.0000 | 0.0000 (42/300, 14.0%) | -465.6200 | 452.1800 | 917.8000 | 173.28 | 153.21 | 0.0564 | 0.7537 |
...
全脳過同期てんかん発作時における、脳領域間の結合変化を検証しました。
データソースは [result.000_2_4_stiffness_diff.analysis.csv](../../../../samples/Sample_9_fMRI_Seizure/output_data/result.000_2_4_stiffness_diff.analysis.csv) です。
...
### ① 最大スペクトル半径（安定性評価）
本システムにおける最大スペクトル半径は、発作Onsetである t=30 以降、位相同期の強制引き込みによって完璧に **`1.0000`** で高止まり（厥逆）し、全脳のエネルギー循環が自律的制御限界を超えて同期飽和していることを実証しています。

### ② 多階ヤコビアン（1st, 2nd, 3rd-order）による過同期伝播解析
データソースは [result.003_1_3_jacobian_*.analysis.csv](../../../../samples/Sample_9_fMRI_Seizure/output_data/result.003_1_3_jacobian_1st.analysis.csv) です。
