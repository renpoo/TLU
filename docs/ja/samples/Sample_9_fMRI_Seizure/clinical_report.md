...
* **根本原因 (安定性の評価):** タイムステップ **`t=30` (10:05:00)** より、側頭葉（`03_Temporal_Lobe`）を起点として、他領域との間の流入・流出流量が全く同一のサイン波信号によって強制駆動（Forced Oscillation）され、全エッジの流量が小数点以下2桁まで完全に一致する位状態（Phase Lock）に陥ったことが主原因です。これにより最大スペクトル半径は安定限界である **`1.0000`** に固定化されています（厥逆）。
* **総合体質診断（健康状態）:**
...
* **要改善点と介入アドバイス:**
  * **肩こり（滞り）の特定:** 最も機能伝達遅延（粘性）が高い領域は **前頭前皮質 (`02_Prefrontal_Cortex`)** であり、平均粘性 `10015.93`、発作の前兆期である **`10:08:30` (t=51)** にピーク値 `10022.91` に達する深刻な「情報の滞り」が発生しています。
...
* **肩こり（粘性 `viscosity_C`）:** 前頭前皮質（`02_Prefrontal_Cortex`）が平均 `10015.93` と、顕著な伝達遅延（肩こり）を発生させています。
...
#### 📐 脳機能過同期における「分散累積比率不変（PCAの死角）」の数学的背景
てんかん発作のような全脳過同期アノマリーにおいては、脳活動全体の主成分PCAの寄与率（PC1占有比率）が、発作前も発作中も `37%` 前後のほぼフラットな一定値で推移し、警告シグナルを全く発しません。
これは、全領域が全く同一のサイン波を共有したため、多次元空間内の共分散の相対的なばらつき（比率）自体は変化しないためです。これが「PCAの死角」です。
本システムはこの限界を克服するため、脳波のフラクタル性質を示す **1/fゆらぎの喪失**（ホワイトノイズからサイン同調波への移行による位相コヒーレンスの最大化）、およびエッジ接続の**最大スペクトル半径 $\rho \ge 0.75$ 飽和ロック**を検知することで、統計モデルをすり抜ける過同期アノマリーを正確に同定しています。
...
本システムにおける各脳領域の平均粘性分布から、第3四分位数 Q3 しきい値（**`10002.2033`** 以上）の範囲にある高粘性（滞り）ノード群を複数特定しました。データソースは [result.000_1_1_filter_dynamics.analysis.csv](../../../../samples/Sample_9_fMRI_Seizure/output_data/result.000_1_1_filter_dynamics.analysis.csv) です。

* **`02_Prefrontal_Cortex`** (平均粘性: **`10015.9306`** / 最も滞る時期: **`t=51 (10:08:30)`**)
  - 数理的解釈: 局所粘性時系列トレンドのヒートマップ（`000_1_7_1__viscosity_trend.png`）において、発作前兆期（t=51）に前頭前皮質へ急激な神経伝達遅延（粘性）が集中・発生している様子が視覚的に実証されています。
* **`03_Temporal_Lobe`** (平均粘性: **`10002.2033`** / 最も滞る時期: **`t=59 (10:09:50)`**)

### ② 特効穴（ツボ）および禁忌（避けるべき介入）の特定
介入歪みエネルギー（`ik_strain_energy`）の平均分布から、第1四分位数 Q1 しきい値（**`0.4411`** 以下）および第3四分位数 Q3 しきい値（**`0.4616`** 以上）の範囲に基づいて、ツボと禁忌の領域を抽出・序列化しました。

#### 🎯 特効穴（ツボ）の範囲 (歪みエネルギー $\le$ Q1)
介入による正常細胞への二次損傷（反発ストレス）を最小化しつつ、全脳の同期ロックを最も効果的に脱同調・復元するための推奨介入領域です。
1. **`02_Prefrontal_Cortex`** (平均歪みエネルギー: **`0.4399`**)
2. **`04_Visual_Cortex`** (平均歪みエネルギー: **`0.4411`**)

#### 🚫 避けるべき介入（禁忌）の範囲 (歪みエネルギー $\ge$ Q3)
発作の過同期ループをさらに激化（反発ストレス最大）させ、脳の健全な情報処理能力を崩壊させる致命的な禁忌領域です。
...
本診断（てんかん過同期発作の発生）を覆すために実査で必要な境界外の客観的証拠は以下の通りです。
1. **臨床脳波(EEG)生記録の提示:**
...
2. **臨床発作の目視観察ビデオログの検証:**
   fMRI測定中に撮影された、被験者の「目視観察ビデオログ原本（管理者または医師の署名入り）」を解析し、発作Onsetとされた t=30 以降において、てんかん発作特有の臨床徴候（意識障害、強直・間代発作、自動症など）が一切認められず、被験者が完全に正常な覚醒・認知状態であったことが臨床的に証明された場合。
