...
* **💡 グラフの読み方（経理・監査・コンサルタント向け）:** 
  * 正常系では赤色の線（スペクトル半径）が低く安定していますが、本サンプルでは異常閾値（0.6）を完全に突き破って `1.0` に張り付いています。金融においては「特定の口座間で架空の売上を回し続ける循環取引」を意味しますが、神経モデルにおいては「脳の様々な領域が、まるで一つの塊のように同じリズムで一斉に発火し続ける（てんかんの同期発作）」状態を視覚化しています。
...
* **相対的自由エネルギー比率:** `-5.5882` (異常閾値 `< -0.1` を致命的に超過)
* **【🟢 正常系（Sample 0）のベースライン】**
