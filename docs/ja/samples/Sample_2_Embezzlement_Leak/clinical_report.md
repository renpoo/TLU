...
* **総合体質診断（健康状態）:** 
  本システムは「体格（質量）」が平均 `181818.18` と低下傾向にあり、免疫力（自由エネルギー）も流出に伴い平均 `2944447.97` と侵食されています。流出月（2月, 3月, 8月, 9月, 11月）を中心に体内のエネルギーバランス（エントロピー `1.4925`）が崩れ、システム全体が柔軟性を失った「慢性硬直（最大結合剛性 `1.44e-09` の動脈硬化）」と、後半ステップにおける壊滅的な共振（ノッキング）を引き起こしています。これは、たった 0.05% の資金漏洩（小さな傷口）がシステム全体を機能停止（資金ショート）へ追いやる極めて重篤な状態です。高次運動学の Jerk (加加速度) および Snap (加加加速度) には流出発生ステップで激しいノッキング（発振）が確認されます。
* **要改善点と介入アドバイス:** 
  - **肩こり（滞り）の特定:** 最も決済タイムラグ（粘性）が高い実質ノードは 在庫（`04_ACC_Inventory（棚卸資産）`）で平均粘性 `52569.22`、特に期末の **`2020-12`** にピーク値 **`57275.08`** に達する滞留が発生しています。
  - **ツボ（改善点）と禁忌（避けるべき介入）の特定:** 不正流出（出血）を差し引いたうえで、システムの免疫力（自己治癒力）を回復させるためのツボは 売上原価（`02_ACC_COGS（売上原価）` / 歪みエネルギー最小 `3.65`）です。逆に、地代家賃（`06_ACC_Rent_Exp（地代家賃）` / 歪みエネルギー最大 `8.35`）の削減介入は、組織に最大の痛み（構造破壊）を生むため避けてください（禁忌）。
...
* **肩こり（粘性 `viscosity_C`）:** 在庫（`04_ACC_Inventory（棚卸資産）`）が平均粘性 `52569.22`、ピーク期 12月に `57275.08` と慢性的な遅延（肩こり）を引き起こしています。
* **脈の急変・過渡的ゆらぎ（Jerk `jerk_j` & Snap `snap_s`）:**
  - **数理的解釈:** 横領による不連続な質量流出が発生するタイミング（2月, 3月, 8月, 9月, 11月）で、高次時間微分の Jerk および Snap に非常に鋭い孤立インパルス（決済ショック波）が発生しています。特に、キャッシュが底をつき始める後半ステップにおいて、システムがしなやかさを失った「ノッキング共振（発振状態）」を引き起こしていることが観察されます。
...
本システムにおける対流動的データ（状態 `state_X`, 速度 `velocity_v`, 加速度 `acceleration_a`, 局所粘性 `viscosity_C`、および加加速度 `jerk_j`、加加加速度 `snap_s`）の記述統計量を以下に示します。データソースは [result.000_1_1_filter_dynamics.analysis.csv](../../../../samples/Sample_2_Embezzlement_Leak/output_data/result.000_1_1_filter_dynamics.analysis.csv) です。
...
流出が発生するタイミングでの決済結合関係の動的硬化を時間差分ヒートマップで検証しました。
データソースは [result.000_2_4_stiffness_diff.analysis.csv](../../../../samples/Sample_2_Embezzlement_Leak/output_data/result.000_2_4_stiffness_diff.analysis.csv) です。
...
### ① 最大スペクトル半径（安定性評価）
本システムにおけるスペクトル半径 $\rho$ は、流出ステップにおいて大きな変動を示しませんが、外部流出先への一方通行ルートが確立されているため、システム全体のエネルギーは Neumann 級数が収束せずに漏れ出し続ける不安定状態（自己治癒不能）にあります。

### ② 多階ヤコビアン（1st, 2nd, 3rd-order）による漏洩経路解析
データソースは [result.003_1_3_jacobian_*.analysis.csv](../../../../samples/Sample_2_Embezzlement_Leak/output_data/result.003_1_3_jacobian_1st.analysis.csv) です。
...
本システムにおける各実質ノードの平均粘性分布から、第3四分位数 Q3 しきい値（**`40246.5119`** 以上）の範囲にある高粘性（滞り）ノード群を複数特定しました。

* **`04_ACC_Inventory`**:
  - 平均粘性値: **`52569.2200`**
  - 最も滞る時期: **`2020-12`**（ピーク粘性値: **`57275.0845`**）
  - 数理的解釈: 資金ショートおよび取引ブロックに伴う滞留が発生し、局所的な気の滞り（還流や硬直）を慢性的に誘発する原因となっています。
* **`03_ACC_Cash`**:
  - 平均粘性値: **`45680.1844`**
