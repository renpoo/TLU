...
主成分分析（PCA）において、2020-03 (`t_idx=2`) の PC0 固有値は `6.6203e9` に達し、説明分散比率は **`100.0%`** となっており、PC1ベクトルは `ACC_Accounts_Receivable` (`0.6221`) と `ACC_Cash` (`-0.5138`) に支配されています。これは、流出の衝撃が主要な主成分軸を完全に占拠し、システムに極端な偏向が生じていることを示します。
...
### 4.4. 3Dミクロ情報幾何学の尖塔スパイク警告
3Dプロットは、統計AIモデルが看過した「ゼロ・トゥ・ワン異常」の決定的な発生源を特定します。
