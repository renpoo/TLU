...
* **特定された証拠:**
`grep` によるトランザクション・レベルの追跡の結果、貸方（流出元）にのみ金額が記録され、借方（流入先）が存在しない以下の「Embezzlement_Leak」トランザクション群が特定されました。TLUの前処理エンジンは、このように「行き場を失った資金（$0.0の欠落）」を検知すると、数学的な質量保存則を維持するために**自動的に `UNKNOWN_LEAK`（使途不明の流出）という特設科目を生成**し、そこに差額を強制的に流し込みます。先ほどのトポロジー図や3Dサーフェスグラフにおける異常なスパイクは、まさにこの特設科目に質量が不自然に蓄積された物理的痕跡です。
