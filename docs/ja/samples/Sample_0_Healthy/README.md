...
1. **3D外力マップ（平穏な波形）:**
   Sample 2（横領）では後半に `1e9` スケールの破滅的な大波（ノッキング）が発生しましたが、この Sample 0 の外力マップは1年間を通じて非常に穏やかな起伏しか観測されません（3Dグラフ縦軸のスケールにご注目ください。元データにおける数千〜一万ドル規模の巨大な取引入力に対し、出力される外力は極めて軽微な数値に抑え込まれています）。これは、システム（健全なサスペンション）が外部からの巨大な入力エネルギーを完璧に吸収・分散できている物理的証拠です。
