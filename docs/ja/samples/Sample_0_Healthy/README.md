...
* **ドメイン的証拠:**
  * 貸借対照表（B/S）は総資産 216,622.48 と負債・純資産の合計が完全に一致しており、質量保存則（貸借一致の原則）が維持されています。
...
* **💡 正常系の読解:** 上段の「System Conservation Residual（質量の絶対残差）」が完全に `0.0` の地平に張り付いています。これは「借方と貸方が1円の狂いもなく一致し、システム外へ消失した（漏れた）質量が一切ない」という貸借一致の原則（質量保存則）が完璧に機能していることを証明しています。異常系（Sample 3, 4など）ではここに鋭いスパイクが発生します。また、下段の「Statistical Anomaly (Z-Score)」も低いレベルで安定しており、統計的な外れ値が存在しないことを示しています。
...
異常系（Sample 2など）で強力な証拠となった「外力の異常共振」と「剛性行列の絶対硬直（Rigid Lock）」が、「単なる数理モデルの過敏な反応」や「分析者の過剰な解釈」ではないことを証明するためには、この Sample 0（完全な健康体）がどのような波形と内部構造を示すかを確認する必要があります。
...
1. **3D外力マップ（平穏な波形）:**
   Sample 2（横領）では後半に `1e9` スケールの破滅的な大波（ノッキング）が発生しましたが、この Sample 0 の外力マップは1年間を通じて非常に穏やかな起伏しか観測されません（算出された数値の定義域・値域の桁数（オーダー）にご注目ください。元データにおける数千〜一万ドル規模の巨大な取引入力に対し、出力される外力は極めて軽微な桁数へと適切に抑制・吸収されています）。これは、システム（健全なサスペンション）が外部からの巨大な入力エネルギーを完璧に吸収・分散できている物理的証拠です。
...
* **物理的結論:** この Sample 0 の結果こそが、「TLUの出力した異常波形が、数理モデルの暴走でも、分析者の過剰解釈でもない」ことの最大の証明です。健康なシステムでは、共振も絶対硬直も一切発生しません。純然たる数理解析器であるTLUは、「本当にシステム構造が破壊された時」にだけ、その事実を異常波形として出力するように厳密に設計されているのです。
...
* **💡 正常系の読解:** システム全体にわたる統計的ショック（Z-Score）の3Dサーフェスです。一年を通じて、全ノードで目立ったスパイクが一切存在せず、「完全に平坦で凪いだ海（Z-Score ≈ 0）」を保っていることが視認できます。この「無風状態」が、他のサンプル（局所的な不正やエラー）で発生する鋭いスパイクを検知するための絶対的なベースライン（原器）となります。
