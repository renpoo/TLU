...
本サンプル（**Sample_8_fMRI_Stroke**）に対する自動判定結果です。純粋な物理指標がドメイン特有の異常をどう捉えるかを解説します。
...
### 4.3. 自由エネルギー崩壊と「質量なきネットワーク」の脆弱性 (Heat Death & Lack of Mass)

Sample 8 では、病変が発生する中盤（TR=150付近）から突如としてエントロピー損失（$T \Delta S$：赤色の層）が激増し、自由エネルギーがマイナス領域（-212,000）へと深く沈み込んでいる。

**【物理的・生物学的解釈】**
直前の株式市場（Sample 6, 7）では、参加者に「巨大な保有株式（質量＝$U$）」を与えたことで、システムの自由エネルギーは枯渇せず、暴落やフラッシュクラッシュに耐えることができた。
しかし、**脳の神経細胞（ニューロン）は、酸素やグルコースの「静的な貯蔵庫（質量）」をほとんど持たない。** 脳は、絶え間なく供給される血流（速度・フラックス＝$v$）に100%依存して生きている「質量なき流体ネットワーク」である。
そのため、運動野への血流が遮断（Stroke）されると、依存する内部エネルギー（蓄え）がないため、摩擦熱（エントロピー）だけが相対的に暴走し、即座に自由エネルギーがマイナス無限大に沈み込む（細胞壊死）。

金融市場が「洗練されたアルゴリズムで熱的死を防げる」のに対し、生命体は「常に熱的死と隣り合わせの綱渡りをしている」ことが、TLUの**全く同一の熱力学方程式**によって見事に証明された。
