...
* **💡 具体例（サンプル・データより）**:
  * **Sample_1（循環取引/Wash Trade）**: 数学的に「スペクトル半径が1.0に到達する」ということは、減衰することのない「無限の反響（フィードバック・ループ）」が形成されたことを意味します。悪意のあるアクターが売上を架空のダミー会社間でA→B→C→Aと「循環取引」した場合、資金フローの遷移行列の最大固有値は正確に1.0へと漸近します。これはシステムが自己崩壊（死の螺旋）の臨界点に達したことを示す、循環取引の最も決定的な証拠です。
