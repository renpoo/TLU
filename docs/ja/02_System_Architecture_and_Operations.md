...
TLUの可視化機能は、レポート内の多様な数理解析グラフの視認性を統一するため、JSONファイルによるデザインテーマ管理と、解析項目ごとの特性に応じたカラーマップを採用しています。

### 解析ごとのカラーパレットの使い分けと物理的意味

* **発散型カラーマップ (Diverging CMAPs: `RdBu_r`, `coolwarm` 等)**:
    ネット流量のプラス・マイナスや、目標からの過不足の差分など、基準（ゼロ）から二方向へ対称に広がる変動の可視化に用います。
* **順序型カラーマップ (Sequential CMAPs: `inferno`, `magma`, `viridis` 等)**:
    エントロピー（乱雑さ）、局所温度（摩擦損失）、KL Drift（情報の断裂）など、強さや密度の連続的なグラデーションを立体可視化する際に適用されます。
* **警告・異常値のハイライト配色**:
    健全なデータ点は落ち着いたトーン（青や緑）で表現し、Z-Scoreの臨界閾値を超えたアノマリーはビビッドな赤やサーモンピンクなどの警告色（星マーク等）で強調表示し、読者の注意を喚起します。
