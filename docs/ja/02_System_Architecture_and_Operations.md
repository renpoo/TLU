...
Tensor-Link Utility (TLU) は、単一の分析ツールではなく、データの取り込みから物理変換、可視化、そしてAIエージェントによる自動診断までを一気通貫で処理する**「自律的フォレンジックプラットフォーム」**です。
...
TLUの全体的なデータおよび診断処理の流れは、以下の多層パイプラインとして定義されています。

```mermaid
graph TD
    A[Raw Data Source: 仕訳ログ, 交通GPS, fMRI BOLD, 市場注文簿] -->|データ抽出| B[Data Projection Layer]
    B -->|グラフ・行列化| C[Physics Engine Layer]
    C -->|6つの物理フィルター適用| D[Forensic Filter Layer]
    D -->|物理・数学不変量の計算| E[Visualization Layer: 3Dプロット, 剛性行列, TS図]
...
2.  **物理エンジン層 (Physics Engine Layer):** クラシカル剛性、熱力学ポテンシャル、情報多様体、LQRフィードバック、波動コヒーレンスの6つの物理コアが計算を行います。
3.  **可視化層 (Visualization Layer):** 生成された時空間データを、直感的な 3D 空間リボン、マトリクス、T-SグラフなどのPNG画像としてレンダリングします。
4.  **AIメタ診断層 (LLM Meta-Diagnosis Layer):** 統合されたLLMが最高メタレベルプロンプト（`LLM_Diagnostic_Manual.md`）に準拠し、物理エンジン出力からデータに裏打ちされた客観的カルテを日本語で自動生成します。
...
*   **3D局所温度マッピング (Thermal Gradient):** 絶対零度（青）から中温（緑）、高温の摩擦・ボラティリティ領域（黄・赤）まで、温度の偏在（コールドアイランド・ヒートアイランド）をグラデーションで立体可視化します。
...
*   **物理的帰結 (t=40 / W41):** 固有ベクトル loadings がこの2者に `0.72` と `-0.68` で異常偏在し、PC1説明比率が **`99.67%`** に跳ね上がって剛性が完全ロック（Stiffness Lock）。さらに二者間の位相差がゼロで完全同期し、自由エネルギー歪度が **`-2.72`** へと急降下します。

---

## 5. TDDシミュレーションとLQR制御モデル (TDD & LQR Control)

TLUは、テスト駆動開発（TDD）に完全に準拠しており、シミュレータが動作した際に期待される物理パラメータの範囲をしきい値としてアサーション（プログラム検証）を行います。
