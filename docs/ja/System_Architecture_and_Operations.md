...
* **金融（仕訳）ドメイン:**
  ...
  * **エッジ:** 勘定間の仕訳取引金額（移動質量）。
* **ERP原価計算・管理会計ドメイン:**
  * **ノード:** 勘定科目 ✕ 部門コードの部勘結合（`ACC_Finished_Goods_DPT_Prod_A` など）。
  * **エッジ:** 製造間接費配賦・原材料投入・労務費帰属仕訳（動的活動質量）。
* **都市交通ドメイン:**
  * **ノード:** 交差点。
...
#### 3. ERP原価配賦ジェネレータ (`src/filters/_0_0_generate_dummy_journal_erp.py`)

* **モデル選択 (`--allocation-mode`):** `traditional` (労務時間比例一括配賦), `abc` (固定3活動プール配賦), `tabc` (動的熱力学配賦)。
* **動的ボラティリティ散逸 ($\alpha(t)$):** 日次活動負荷の動的ゆらぎ ($CV_{\text{activity}}$) から物理的摩擦熱 $\alpha(t)$ を自律算出し、製品原価から物理的に独立隔離します。
