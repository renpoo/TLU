...
This generator simulates the daily accounting journal entries of a medium-sized enterprise over a multi-year period. It models departments (Sales, R&D, Ops, etc.) and accounts (Cash, Accounts_Receivable, AP, etc.) as nodes in a network.
...
* **Causal Event Queues (Viscosity):** A sale today (`Sales -> Accounts_Receivable`) schedules a collection event 30-60 days in the future (`Accounts_Receivable -> Cash`). This temporal delay creates natural "Viscosity" in the Phase Space mechanics.
...
5. **Wash Trading (Circular Transactions):** A series of transactions specifically designed to form a closed loop (e.g., `Cash -> Accounts_Receivable -> Sales -> Cash`). This breaks the natural Directed Acyclic Graph (DAG) structure of the accounting ledger and is instantly flagged by the **System Stability (Spectral Radius)** filter.
