...
1. **`000_0_Basic_Statistics.md`**: 基礎体力（B/S, P/L）と統計的な脆弱性（Z-Score, ファットテール）の判定。
2. **`000_1_Kinematics.md`**: 粘性（肩こり・摩擦）、慣性（メタボ）、位相空間の軌道に基づく動的体質の判定。
3. **`000_2_Structural_Stiffness.md`**: システムの硬直化（Rigidity）と主成分の入れ替わりによる構造転換（Regime Shift）の判定。
4. **`001_Thermodynamics.md`**: エントロピーと自由エネルギーによる「気の滞り（摩擦）」や「人工的同期」、Lag行列による局所的遅延の判定。
5. **`002_Information_Geometry_Forensics.md`**: トポロジーの変遷による経絡の断裂（Stress Zeroing）、質量欠損（Leakage/出血）、病巣（Micro Forensics）の特定。
6. **`003_004_Control_Theory.md`**: FK/IKシミュレーション結果とスペクトル半径による「脈の暴走（架空循環・ロック）」の判定、およびツボ（経絡秘孔）に対する動的治療（LQR）の特定。
7. **`005_Wave_Mechanics.md`**: フラクタルノイズ（1/fゆらぎ）の喪失による「死の静寂」や「人工的介入」、位相のズレの判定。
...
* **適用例:** `000_0_Basic_Statistics` で「売上が右肩上がりで健康」という所見が出ても、`000_1_Kinematics` で「粘性が極度に高い（深刻な肩こり）」、あるいは `002_Information_Geometry_Forensics` で「Leak Ratioがゼロではない（出血）」という所見が出た場合、売上の成長は「健康」ではなく「病気による腫れ（Metabolic Bloat）」として断固として切り捨て、メタ所見を「Critical Condition（重篤な出血と血栓）」へと昇華させなければなりません。

### 2.2 矛盾の止揚 (Synthesizing Contradictions)
* **ルール:** 異なるモジュール間で一見矛盾する結果が出た場合、それらが「複合病態（Composite Pathology）」を構成していないか疑うこと。
* **適用例:** `003_004_Control_Theory` で「スペクトル半径 1.0（無限のフィードバックループ）」が起きて活発に取引が回っているように見えても、`002_Information_Geometry_Forensics` で「Edge Stressが0.0（経絡の切断）」が起きていた場合。これは「活発に循環している」のではなく、「パイプが切断された状態で、同じ場所で無理やり空回りしている（Gridlock / 交通麻痺・凍結）」というメタ所見へと統合します。
...
3. **動的治療の提案 (Dynamic Treatment):** `003_004_Control_Theory` で特定した「ツボ（Acupressure Point）」に対して、「売上を増やす」といった物理的に不可能な絶対量の操作ではなく、「位相（タイミング）をずらす」「粘性（手作業の摩擦）を減らす」といった**動的プロパティの改善提案**を必ず行うこと。
