...
* **Signal Tuning Range:** Intersections in the bottom 25% of intervention strain energy—**"Shijo-Karasuma (21_ShijoKarasuma)"**, **"Shijo-Shinmachi (24_ShijoShinmachi)"**, and **"Nijo-Karasuma (11_NijoKarasuma)"**—allow adjustments with minimal secondary congestion.
  * **Advice:** Intersections with the lowest secondary congestion propagation (IK strain energy) are prioritized. Specifically, when regulating traffic at boundary terminal nodes (Jing-Well nodes) directly connecting to the suburban road grid (e.g., edge intersections), the severe congestion spillback (External Backlash) forced onto suburban roads must be constantly evaluated. Aggressive inflow restrictions (sedation/泻) will trigger negative feedback loops, causing logistics delays and urban vitality decline. Rather than simple traffic throttling, a symbiotic package of interventions—such as strengthening multi-modal public transit connectivity or expanding park-and-ride facilities—should be integrated to maintain balanced traffic homeostasis.
