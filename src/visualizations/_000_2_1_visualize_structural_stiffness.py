...
def render_stiffness_frame(t_idx, K_matrix, symlog_norm, axis_labels, time_labels, cmap_stiffness, text_col, base_name, ext, out_dir):
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    import seaborn as sns
...
def main():
    ...
    for t_idx in t_indices:
        ...
        K_matrix_list[t_idx] = K_matrix

    if len(t_indices) <= 1:
        for t_idx in t_indices:
            render_stiffness_frame(t_idx, K_matrix_list[t_idx], symlog_norm, axis_labels, time_labels, cmap_stiffness, text_col, base_name, ext, args.out_dir)
    else:
        max_workers = min(len(t_indices), os.cpu_count() or 4)
        tasks = []
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            for t_idx in t_indices:
                tasks.append(
                    executor.submit(render_stiffness_frame, t_idx, K_matrix_list[t_idx], symlog_norm, axis_labels, time_labels, cmap_stiffness, text_col, base_name, ext, args.out_dir)
                )
