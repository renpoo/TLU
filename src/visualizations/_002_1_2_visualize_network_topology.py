...
def render_topology_frame(t, df_t, N, pos, global_max_weight, global_max_stress, global_vmax_node, global_vmax_gross,
                          cmap_node, cmap_edge, c_outlier_text, canvas_bg, text_col, bg_col, edge_col,
                          idx_to_label, time_labels, args_top_k, args_max_legend, args_filename, args_out_dir):
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    import networkx as nx
    ...
    top_k_indices = np.argsort(np.abs(net_flux))[-args_top_k:].tolist()
    ...
    handles, labels = [], []
    display_count = min(N, args_max_legend)
    for i in range(display_count):
        handles.append(mpatches.Patch(color='none'))
        labels.append(f"{i:02d} : {idx_to_label.get(i, f'Node_{i}')}")
    if N > args_max_legend:
        handles.append(mpatches.Patch(color='none'))
        labels.append(f"... and {N - args_max_legend} more")
...
def main():
    ...
    # Sequential drawing loop for each time step with ProcessPoolExecutor
    if len(t_targets) <= 1:
        for t in t_targets:
            df_t = df[df['t_idx'] == int(t)]
            render_topology_frame(int(t), df_t, N, pos, global_max_weight, global_max_stress, global_vmax_node, global_vmax_gross,
                                  cmap_node, cmap_edge, c_outlier_text, canvas_bg, text_col, bg_col, edge_col,
                                  idx_to_label, time_labels, args.top_k, args.max_legend, args.filename, args.out_dir)
    else:
        max_workers = min(len(t_targets), os.cpu_count() or 4)
        tasks = []
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            for t in t_targets:
                df_t = df[df['t_idx'] == int(t)]
                tasks.append(
                    executor.submit(render_topology_frame, int(t), df_t, N, pos, global_max_weight, global_max_stress, global_vmax_node, global_vmax_gross,
                                    cmap_node, cmap_edge, c_outlier_text, canvas_bg, text_col, bg_col, edge_col,
                                    idx_to_label, time_labels, args.top_k, args.max_legend, args.filename, args.out_dir)
                )
