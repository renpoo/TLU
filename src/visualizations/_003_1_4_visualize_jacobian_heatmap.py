...
def main():
    ...
    # 3. Generate heatmap for each target step
    for t in target_t_list:
        df_t = df[df['t_idx'] == t]
        
        # Build N x N Jacobian Matrix for step t
        ...
        # Set dynamic color bounds based on order
        # 1st-order might have higher values, while higher orders tend to decay
        ...
        order_str = {1: "1st (Direct)", 2: "2nd (1-hop)", 3: "3rd (2-hop)"}.get(args.order, f"{args.order}th")
        ...
        # Build output filename matching the Neumann series visualizer naming scheme
        base_name, ext = os.path.splitext(args.filename)
        # Handle format template in default filename
        formatted_base = base_name.format(order=args.order)
