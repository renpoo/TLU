...
def main():
    ...
    # 2. Create monthly labels
    dt_col = pd.to_datetime(df[col_time])
    df['Month'] = dt_col.dt.year.astype(str) + "-" + dt_col.dt.month.astype(str).str.zfill(2)

    # 3. Aggregate Debits and Credits by Entry_ID
    debits = df[df['Debit'] > 0].groupby('Entry_ID').agg({'Account_Name': 'first', 'Dept_Name': 'first', 'Debit': 'sum', 'Month': 'first'}).reset_index()
    debits = debits.rename(columns={'Account_Name': 'Tgt_Account', 'Dept_Name': 'Tgt_Dept', 'Debit': 'Debit_Amt'})

    credits = df[df['Credit'] > 0].groupby('Entry_ID').agg({'Account_Name': 'first', 'Dept_Name': 'first', 'Credit': 'sum', 'Month': 'first'}).reset_index()
    ...
    # Resolve Month (take from debit, if missing take from credit)
    edges['Month'] = edges['Month_dr'].combine_first(edges['Month_cr'])
