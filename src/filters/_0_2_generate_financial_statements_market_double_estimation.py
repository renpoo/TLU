#!/usr/bin/env python3
# ==========================================
# _0_2_generate_financial_statements_market_double_estimation.py
# TLU System: Utility & Simulation Layer
# Category: Financial Statement Generator with Dual Conservation & Flow Estimation
# ==========================================
...
def setup_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate MTM Financial Statements with Flow Estimation")
...
def main():
    ...
    # 3. Read Stream from true_stream (actual bank records) to simulate the true state
    try:
    ...
    # Simulation loop using df_true (tracking true bank balances)
    cum_inflow_true = 0.0
    cum_outflow_true = 0.0

    # To calculate current week's estimated leak from discrepancy change
    prev_discrepancy = 0.0
    cum_estimated_inflow = 0.0
    cum_estimated_outflow = 0.0
    ...
    for w in weeks:
        ...
        # 3. Dynamic Flow Estimation via Accounting Discrepancy (Estimation Logic)
        # Without mapping external flows, the discrepancy on B/S would be:
        # Discrepancy = Assets - (Liabilities + Initial_Equity)
        discrepancy = round(assets - (liabilities + equity), 2)
        
        # Weekly delta in discrepancy represents the estimated flow for the current week
        weekly_delta = round(discrepancy - prev_discrepancy, 2)
        prev_discrepancy = discrepancy

        if weekly_delta > 0:
            cum_estimated_inflow = round(cum_estimated_inflow + weekly_delta, 2)
        elif weekly_delta < 0:
            cum_estimated_outflow = round(cum_estimated_outflow + (-weekly_delta), 2)

        # Net income is the cumulative discrepancy
        net_income = discrepancy

        total_equity = equity + net_income
        total_liab_eq = liabilities + total_equity
        is_balanced = abs(assets - total_liab_eq) < 0.01

        # We populate revenue and expense for visualization compatibility
        # revenue -> cum_estimated_inflow (green bar)
        # expense -> cum_estimated_outflow (ACC_Estimated_External_Leak)
        report = {
            'week': w,
            'assets': assets,
            ...
            'revenue': cum_estimated_inflow,
            'expense': cum_estimated_outflow,
            'is_balanced': is_balanced,
            'bs_items': bs_items,
            'pl_items': [
                ('Estimated_Outside_Inflow_Revenue', 'Revenue', cum_estimated_inflow),
                ('ACC_Estimated_External_Leak', 'Expense', cum_estimated_outflow)
            ],
            'tb_items': tb_items
        }
    ...
    with open(args.output, 'w') as f:
        f.write("# TLU Mark-to-Market Financial Statements Report (State Estimation / Reconstructed Flows)\n\n")
        f.write("> *This report demonstrates the estimation and reconstruction of hidden external flows using the B/S discrepancy.*\n\n")
        
        final = weekly_reports[-1]
        f.write("## 1. Total Period Summary (Cumulative)\n\n")
        f.write(f"**Period End:** {final['week']}\n")
        f.write(f"**Status:** {'✅ BALANCED (via Reconstructed Flows)' if final['is_balanced'] else '❌ UNBALANCED'}\n\n")
        ...
        for acc, cat, bal in final['pl_items']:
            f.write(f"| {acc} | {cat} | {bal:,.2f} |\n")
        f.write("|---|---|---|\n")
        f.write(f"| **Net Income (Reconstructed Net Flow)** | | **{final['net_income']:,.2f}** |\n\n")
    ...
    print("Completed successfully. Balanced financial reports with flow estimation generated.")
