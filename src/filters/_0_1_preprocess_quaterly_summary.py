#!/usr/bin/env python3
# ==========================================
# _0_1_preprocess_quaterly_summary.py
# TLU System: Pre-filtering Layer
# Action: Quarterly Aggregation of Journal Entries (Account:Dept Level)
# ==========================================
import sys
import pandas as pd
...
def main():
    ...
    # 2. Create quarterly labels
    dt_col = pd.to_datetime(df[col_time])
    df['Quarter'] = dt_col.dt.year.astype(str) + "-Q" + dt_col.dt.quarter.astype(str)

    # 3. Aggregate Debits and Credits by Entry_ID
    debits = df[df['Debit'] > 0].groupby('Entry_ID').agg({'Account_Name': 'first', 'Dept_Name': 'first', 'Debit': 'sum', 'Quarter': 'first'}).reset_index()
    debits = debits.rename(columns={'Account_Name': 'Tgt_Account', 'Dept_Name': 'Tgt_Dept', 'Debit': 'Debit_Amt'})

    credits = df[df['Credit'] > 0].groupby('Entry_ID').agg({'Account_Name': 'first', 'Dept_Name': 'first', 'Credit': 'sum', 'Quarter': 'first'}).reset_index()
    ...
    # Resolve Quarter (take from debit, if missing take from credit)
    edges['Quarter'] = edges['Quarter_dr'].combine_first(edges['Quarter_cr'])
